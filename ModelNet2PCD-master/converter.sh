...
ROOT_PATH="/home/rajat/Documents/";

# > Path where the converter (off2pcd) is
PATH_TO_CONVERTER="/home/rajat/Documents/"

# > Original dataset directory
DIR_NAME="ModelNet10/ModelNet10";
...
parse_train()
{
  ...
  for OBJ_FILE in $( ls *.off ); do
    OBJ_NAME=${OBJ_FILE%.*};
    ${PATH_TO_CONVERTER}off2pcd/.build/main -p "${OBJ_NAME}.off" -f -r -s ${STEP_SIZE} -o $TRAIN_PATH/${OBJ_NAME}.${EXTENSION} 
...
parse_test()
{
  ...
  for OBJ_FILE in $( ls *.off ); do
    OBJ_NAME=${OBJ_FILE%.*};
    ${PATH_TO_CONVERTER}off2pcd/.build/main -p "${OBJ_NAME}.off" -f -r -s ${STEP_SIZE} -o $TEST_PATH/${OBJ_NAME}.${EXTENSION}
    rm -rf "${OBJ_NAME}.${EXTENSION}";
  done
  cd .. ;
